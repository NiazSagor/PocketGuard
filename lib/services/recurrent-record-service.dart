...
class RecurrentRecordService {
  ...
  List<Record> generateRecurrentRecordsFromDateTime(
    ...
    final tz.Location patternLocation = getLocation(
      recordPattern.timeZoneName!,
    );

    // 2. Convert the start and end dates to TZDateTime objects
    final tz.TZDateTime startDate = tz.TZDateTime.from(
      recordPattern.utcDateTime,
      patternLocation,
    );
    final tz.TZDateTime endDateTz = tz.TZDateTime.from(
      utcEndDate,
      patternLocation,
    );
  ...
  Future<void> updateRecurrentRecords() async {
    List<RecurrentRecordPattern> patterns = await database
